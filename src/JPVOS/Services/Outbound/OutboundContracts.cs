...
public sealed record PrincipalSmsBinding(
    string PrincipalId,
    string Channel,
    string EndpointE164,
    string Version,
...
public interface IPrincipalSmsBindingResolver
{
    PrincipalSmsBindingResult Resolve(string principalId, DateTimeOffset now);
}

public sealed record GithubExactHeadReviewRequest(
...
{
    Task<SmsSendResult> SendAsync(SmsSendCommand command, CancellationToken cancellationToken);
}

public interface IGitHubExactHeadReader
{
    Task<string> GetHeadShaAsync(string repositoryFullName, int pullRequestNumber, CancellationToken cancellationToken);
}
...
public sealed class InMemoryOutboundReceiptStore : IOutboundReceiptStore
{
    ...
    public Task SaveAsync(OutboundMessageReceipt receipt, CancellationToken cancellationToken)
    {
        lock (_gate) _receipts[receipt.MessageId] = receipt;
        return Task.CompletedTask;
    }

    public Task<OutboundMessageReceipt?> GetAsync(string messageId, CancellationToken cancellationToken)
    {
        lock (_gate) return Task.FromResult(_receipts.GetValueOrDefault(messageId));
    }

    public Task<OutboundMessageReceipt?> FindByProviderMessageIdAsync(string providerMessageId, CancellationToken cancellationToken)
    {
        lock (_gate) return Task.FromResult(_receipts.Values.FirstOrDefault(x => x.ProviderMessageId == providerMessageId));
    }

    public Task<OutboundMessageReceipt?> FindLatestForPrincipalAsync(string principalId, CancellationToken cancellationToken)
    {
        lock (_gate)
        {
            return Task.FromResult(_receipts.Values
    ...
    public Task<bool> TryMarkProviderEventProcessedAsync(string providerEventId, CancellationToken cancellationToken)
    {
        lock (_gate) return Task.FromResult(_providerEvents.Add(providerEventId));
    }
}
